...
class Graph {
  ...
  Graph(int numVertices) {
    ...
    for (int i = 0; i < numVertices; i++) 
    {
      ...
      for (int j = 0; j < numVertices; j++)/* 046267 Computer Architecture - Winter 20/21 - HW #3               */
/* Implementation (skeleton)  for the dataflow statistics calculator */
...
public:
   ...
   unsigned modified_register;
   unsigned weight;
    dep_list(unsigned instruction_index_r,unsigned modified_register_r,unsigned weight_r)
    {
        unsigned instruction_index=NULL;
        unsigned modified_register=NULL;
        unsigned weight=NULL;
    }
};

list<dep_list> depend_list;
list<dep_list>::iterator it;


void build_list(list<dep_list> &depend_list,unsigned int numOfInsts)
{
  ...
  {
          dep_list new_element(i, i+1, i+2);
          depend_list.push_front(new_element);
          cout << it->weight << endl;
  }
  //TRAVERSE
    for (it = depend_list.begin() ; it != depend_list.end() ; it++) 
  {
       //   depend_list.push_front(new_element);
  }
}
              ...
              //OPTIONS
        //depend_list.erase(it);
        //depend_list.push_back();
        //depend_list.pop_back();
...
//*********************************************
  build_list(depend_list,numOfInsts);
//*********************************************
